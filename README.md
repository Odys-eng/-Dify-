# 基于 RuyiDify 的制造业设备维修知识库问答平台

本仓库是课程答辩项目，直接复用 RuyiDify 和 Dify 1.15.0 的 Web、API、Worker、Docker Compose、知识库、RAG、插件与 Agent 能力。项目保留 RuyiDify 品牌，不接入真实硬件；首期重点是维修资料治理、维修问答 Chatflow、来源引用和效果评测。

## 项目状态

- 基线来源：RuyiDify 提交 `93e07bfdeeb507db3c2ab0a309afb15573f5f051`
- 基线复制：仅包含源仓库受 Git 管理的文件，不包含源 `.git`、凭据、运行数据和未跟踪课程材料
- 环境复用：不在同一台电脑上启动第二套同端口服务
- 运行检查：2026-07-20 检查时 Docker Desktop 未运行，因此 Web/API 未做在线验收
- 设计文档：[制造业设备维修知识库问答平台设计](docs/superpowers/specs/2026-07-20-manufacturing-maintenance-qa-design.md)

## 本地启动

首次独立部署时，在 `docker` 目录准备本地环境文件并启动服务：
...
已有 RuyiDify 环境可直接复用，无需在本仓库重复启动。不要提交 `docker/.env`、API Key、Token、密码、Cookie 或 Docker 运行数据。

## 本周交付范围

1. 制造业设备维修示例知识库
2. 维修问答 Chatflow 与结构化提示词
3. 回答来源引用与安全边界
4. 30 道标准评测题和评测报告
5. 课程报告、答辩 PPT、演示脚本和备用视频
